...
\verb+mult(ull,dl,r)+
\end{center}
Here, ``\verb+mult+'' is the kind of the operator (a multiplication-shaped one) and its arguments specify that it should be linked to the relative positions $(-2,1)$ (\verb+ull+ means up-left-left), $(-1,-1)$ (\verb+dl+ means down-left) and $(1,0)$. The order in which the links should be specified is indicated on the figure below:
\[
...
\end{center}
This generates a file \verb+monoid_assoc_l.tex+ which can be used in a \LaTeX{} file like:
\begin{verbatim}
\documentclass{article}
...
Various shapes are available for labels. For example,
\[
...
Here, the \texttt{s} parameter is the shape (currently, only \texttt{ellipse} and \texttt{triangle} are available), the \texttt{d} parameter is the direction of the triangle and the \texttt{c} parameter specifies the color of the triangle.
...
All parameters can be saved in a configuration file named \texttt{strid.conf}. To generate a configuration file, type
\begin{verbatim}
strid --dump-conf
\end{verbatim}
